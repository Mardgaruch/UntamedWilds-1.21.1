...
public class EntityTarantula extends ComplexMob implements ISpecies, INewSkins {
    ...
    // MobType removed in 1.21; no override
    ...
    int dx1 = (int)Math.round(Math.cos(Math.toRadians(entityIn.getYRot() + 90)) * 1.2);
    int dz1 = (int)Math.round(Math.sin(Math.toRadians(entityIn.getYRot() + 90)) * 1.2);
    int dx2 = (int)Math.round(Math.cos(Math.toRadians(entityIn.getYRot() + 90)) * -1.2);
    int dz2 = (int)Math.round(Math.sin(Math.toRadians(entityIn.getYRot() + 90)) * -1.2);
    BlockPos pos_1 = entityIn.blockPosition().offset(dx1, 0, dz1);
    BlockPos pos_2 = entityIn.blockPosition().offset(dx2, 0, dz2);
