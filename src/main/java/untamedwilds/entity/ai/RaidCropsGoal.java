...
public class RaidCropsGoal extends Goal {
    ...
    if (this.taskOwner.isTame() || !this.taskOwner.level().getGameRules().getBoolean(net.minecraft.world.level.GameRules.RULE_MOBGRIEFING)) {
            return false;
        }
    ...
    public void tick() {
        if (this.taskOwner.distanceToSqr(targetPos.getX(), targetPos.getY(), targetPos.getZ()) < 4) {
            ...
            if (block.getBlock() instanceof CropBlock) {
                // Build loot params for modern MC (1.21) API
                net.minecraft.world.level.storage.loot.LootParams.Builder loot = new net.minecraft.world.level.storage.loot.LootParams.Builder((ServerLevel) taskOwner.level()).withParameter(net.minecraft.world.level.storage.loot.parameters.LootContextParams.ORIGIN, net.minecraft.world.phys.Vec3.atCenterOf(this.targetPos));
                List<ItemStack> drops = net.minecraft.world.level.block.Block.getDrops(block, (ServerLevel)this.taskOwner.level(), this.targetPos, null, null, ItemStack.EMPTY);
                if (!drops.isEmpty()) {
