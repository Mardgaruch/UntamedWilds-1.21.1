...
public class Spit extends Projectile {
    ...
    HitResult entityHit = ProjectileUtil.getEntityHitResult(this, start, end, this.getBoundingBox().expandTowards(vec3).inflate(1.0D, 1.0D, 1.0D), this::canHitEntity, vec3.length());
        HitResult hitresult;
        if (entityHit != null) {
            double entityDist = entityHit.getLocation().distanceTo(start);
            double blockDist = (blockHit != null && blockHit.getType() != HitResult.Type.MISS) ? blockHit.getLocation().distanceTo(start) : Double.MAX_VALUE;
            hitresult = entityDist <= blockDist ? entityHit : blockHit;
        } else {
            hitresult = blockHit;
        }
    ...
    protected void onHitEntity(EntityHitResult p_37241_) {
        ...
        if (entity instanceof LivingEntity owner) {
            p_37241_.getEntity().hurt(this.level().damageSources().mobProjectile(this, owner), 1.0F);
        }
