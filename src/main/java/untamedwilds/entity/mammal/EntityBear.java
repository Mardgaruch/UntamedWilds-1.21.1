...
public class EntityBear extends ComplexMobTerrestrial implements ISpecies, INewSkins, INeedsPostUpdate {
    ...
    public void aiStep() {
        ...
        if (this.getAnimation() != NO_ANIMATION) {
            if (this.getAnimation() == ATTACK_BITE && this.getAnimationTick() == 1) {
                this.playSound(ModSounds.ENTITY_ATTACK_BITE.get(), 1.5F, 0.8F);
            }
            ...
            if (this.getAnimation() == IDLE_TALK && this.getAnimationTick() == 1) {
                this.playSound(ModSounds.ENTITY_BEAR_AMBIENT.get(), this.getSoundVolume(), this.getVoicePitch());
            }
    ...
    protected SoundEvent getAmbientSound() {
        return !this.isBaby() ? super.getAmbientSound() : ModSounds.ENTITY_BEAR_BABY_AMBIENT.get();
    }

    protected SoundEvent getHurtSound(@NotNull DamageSource source) {
        return !this.isBaby() ? super.getHurtSound(source) : ModSounds.ENTITY_BEAR_BABY_CRY.get();
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (hand == InteractionHand.MAIN_HAND && !this.isTame()) {
            if (this.level().isClientSide()) {
