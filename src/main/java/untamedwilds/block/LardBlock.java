...
public class LardBlock extends Block {
    ...
    public void fallOn(Level worldIn, BlockState state, BlockPos pos, Entity entityIn, float fallDistance) {
        ...
        // DamageSource.fall() helper removed in newer mappings; use the two-arg overload for causeFallDamage
    if (entityIn instanceof LivingEntity living && living.causeFallDamage(fallDistance, 0.2F, living.level().damageSources().fall())) {
    ...
    private static void showParticles(Entity entity, Level world, BlockPos pos, int p_53990_) {
        if (world.isClientSide) {
            ...
            for(int i = 0; i < p_53990_; ++i) {
                world.addParticle(new BlockParticleOption(ParticleTypes.BLOCK, blockstate), entity.getX(), entity.getY(), entity.getZ(), 0.0D, 0.0D, 0.0D);
            }
        }
    }
}
